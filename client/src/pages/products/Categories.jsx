...
const demoData = [
  { id: 1, name: 'Wallets', parent: '-', products: 45, description: 'Leather wallets' },
  { id: 2, name: 'Belts', parent: '-', products: 32, description: 'Leather belts' },
  { id: 3, name: 'Jackets', parent: '-', products: 18, description: 'Leather jackets' },
  { id: 4, name: 'Shoes', parent: '-', products: 25, description: 'Leather shoes' },
];
...
const Categories = () => {
  ...
  const handleAdd = () => {
    if (!form.name) return;
    setData([...data, { id: data.length + 1, name: form.name, parent: form.parent || '-', products: 0, description: form.description }]);
  ...
  return (
    <div>
      <PageHeader title="Categories" subtitle="Manage product categories">
        <button onClick={() => setModalOpen(true)} className="px-4 py-2.5 bg-orange-500 text-white rounded-lg text-sm font-medium hover:bg-orange-600">+ Add Category</button>
      </PageHeader>
          ...
          <tbody>
            {data.map((row) => (
              <tr key={row.id} className="border-b border-gray-100 hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-gray-700">{row.parent}</td>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title="Add Category" size="md">
        <div className="space-y-4">
          ...
          <FormInput label="Parent Category" type="select" value={form.parent} onChange={(e) => setForm({ ...form, parent: e.target.value })}>
            ...
            {data.map((c) => <option key={c.id} value={c.name}>{c.name}</option>)}
          </FormInput>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-gray-200">
            ...
            <button onClick={handleAdd} className="px-4 py-2.5 bg-orange-500 text-white rounded-lg text-sm font-medium hover:bg-orange-600">Add Category</button>
          </div>
        </div>
      </Modal>
    </div>
